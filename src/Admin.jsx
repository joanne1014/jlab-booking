...
const [todayStr, setTodayStr] = useState(getLocalDate);
  ...
  const handleResetNewPw = async () => {
    ...
    try { await apiCall('reset-via-token', { token: recoveryToken, newPassword: resetNewPw }); showToast('✅ 密碼已重設，請重新登入'); setShowResetForm(false); setRecoveryToken(''); setResetNewPw(''); setResetConfirmPw(''); } catch (err) { setResetPwError(err.message || '重設失敗'); }
  ...
  const openEditSvc = async (svc) => {
    ...
    setEditSvcForm(form);
    if (svc?.id) { try { const v = await sbGet(`service_variants?service_id=eq.${svc.id}&order=sort_order`); setEditSvcVariants(v || []); } catch (_) { setEditSvcVariants([]); } } else { setEditSvcVariants([]); }
    setEditSvc(svc || 'new');
  };
  ...
  const saveSvc = async () => {
    ...
    try {
      let serviceId = f.id;
      if (f.id) { const { id, created_at, ...rest } = f; await sbPatch(`services?id=eq.${id}`, rest); setSvcList(prev => prev.map(s => s.id === id ? { ...s, ...rest } : s)); }
      else { const { id, created_at, ...rest } = f; const data = await sbPost('services', [rest]); if (data?.length) { serviceId = data[0].id; setSvcList(prev => [...prev, ...data]); } }
      if (serviceId && editSvcVariants.length > 0) {
        if (f.id) { const origVars = await sbGet(`service_variants?service_id=eq.${serviceId}&select=id`) || []; const existIds = editSvcVariants.filter(v => v.id).map(v => v.id); const toDelete = origVars.map(v => v.id).filter(vid => !existIds.includes(vid)); for (const did of toDelete) await sbDel(`service_variants?id=eq.${did}`); }
        for (const v of editSvcVariants) { if (v.id) { const { id, created_at, ...rest } = v; await sbPatch(`service_variants?id=eq.${id}`, rest); } else { await sbPost('service_variants', [{ ...v, service_id: serviceId }]); } }
      }
      setEditSvc(null); showToast('✅ 已儲存服務'); fetchServices();
  ...
  const saveResched = async () => {
    ...
    } catch (e) { showToast('❌ 更改失敗：' + e.message); }
