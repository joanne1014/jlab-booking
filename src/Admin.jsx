...
export default function Admin() {
  ...
  // ★ FIX: todayStr 改為 state（過 12 點會自動更新）
  ...
  const logChange = (text) => {
    ...
    try {
      sbPost('admin_logs', [{ action: text, performed_by: loginEmail || 'admin' }]);
  ...
  const fetchLogs = async () => {
    setLogLoading(true);
    try {
      const data = await sbGet('admin_logs?order=created_at.desc&limit=100');
      setDbLogs(data || []);
  ...
  const handleChangePw = async () => {
    ...
    } catch (err) { setCpError(err.message || '更改失敗'); }
    setCpLoading(false);
  };
  ...
  const exportCSV = () => {
    ...
    const rows = filteredBookings.map(b => [
      b.booking_date, b.booking_time, b.customer_name, b.customer_phone,
      b.service_name, b.technician_label || '', b.total_price, statusText(b.status)
    ]);
    const BOM = '\uFEFF';
    ...
    a.href = url; a.download = `JLAB_預約_${todayStr}.csv`; a.click();
  ...
  const stats = useMemo(() => {
    const today = todayStr; const monthPre = today.slice(0, 7);
  ...
  // ★ FIX: saveSvc — 新增服務時都可以存 variants
  const saveSvc = async () => {
    const f = editSvcForm;
    ...
    try {
      ...
      if (serviceId && editSvcVariants.length > 0) {
        if (f.id) {
          const origVars = await sbGet(`service_variants?service_id=eq.${serviceId}&select=id`) || [];
          const existIds = editSvcVariants.filter(v => v.id).map(v => v.id);
          const toDelete = origVars.map(v => v.id).filter(vid => !existIds.includes(vid));
  ...
  const toggleSvcActive = async (svc) => {
    try { await sbPatch(`services?id=eq.${svc.id}`, { is_active: !svc.is_active }); setSvcList(prev => prev.map(s => s.id === svc.id ? { ...s, is_active: !svc.is_active } : s)); showToast(svc.is_active ? '已下架' : '已上架'); } catch (e) { showToast('❌ 更新失敗'); }
  };
  ...
  const moveSvc = async (idx, dir) => {
    const arr = [...svcList]; const ni = idx + dir;
  ...
  // ★ FIX: clearFilters 加 setBkPage(0)
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {logLoading ? <p style={{ color: '#999', textAlign: 'center', padding: 40 }}>載入中...</p> : dbLogs.length === 0 ? <p style={{ color: '#999', textAlign: 'center', padding: 40 }}>暫無操作記錄</p> : dbLogs.map((log) => (
                <div key={log.id} style={{ padding: '12px 16px', marginBottom: 8, background: '#faf8f5', borderRadius: 10, borderLeft: '3px solid #b8956a' }}>
                  <div style={{ fontSize: 11, color: '#999', marginBottom: 4 }}>{new Date(log.created_at).toLocaleString('zh-HK')}{log.performed_by && <span style={{ marginLeft: 8 }}>by {log.performed_by}</span>}</div>
        ...
        {tab === 'bookings' && (<>
          ...
          {viewMode === 'list' && (<>
            ...
            <div style={{ background: '#fff', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
              ...
              {/* ★ FIX: 用 pagedBookings 取代 filteredBookings */}
        ...
        {tab === 'frontend' && (<>
          ...
          {svcSubTab === 'services' && (
            <div style={card}>
              ...
              {svcLoading ? <p style={{ textAlign: 'center', color: '#999', padding: 30 }}>載入中...</p> : svcList.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 20px', color: '#999' }}><div style={{ fontSize: 40, marginBottom: 10 }}>🎨</div><div style={{ fontSize: 14 }}>未有服務項目</div><div style={{ fontSize: 12, marginTop: 6 }}>撳「➕ 新增服務」開始設定</div></div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {svcList.map((svc, idx) => (
          ...
          {svcSubTab === 'addons' && (
            <div style={card}>
              ...
              {svcAddons.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 20px', color: '#999' }}><div style={{ fontSize: 40, marginBottom: 10 }}>➕</div><div style={{ fontSize: 14 }}>未有附加項目</div></div>
              ) : (
