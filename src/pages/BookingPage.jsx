...
const api=async(action,payload={})=>{
  const r=await fetch('/api/booking',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,payload})});
...
export default function BookingPage(){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const[sR,aR,stR]=await Promise.all([api('get-services'),api('get-addons'),api('get-staff')]);
        setServices(sR.services||[]);setAddons(aR.addons||[]);setStaffList(stR.staff||[]);
  ...
  const fetchDates=useCallback(async(staffId)=>{
    setDatesLoading(true);setAvailDates([]);setSelDate(null);setSelTime(null);setTimeSlots([]);
    try{
  ...
  const fetchSlots=useCallback(async(date,staffId)=>{
    setSlotsLoading(true);setTimeSlots([]);setSelTime(null);
    try{
  ...
  const pick=id=>{
    setSid(id);setSelStaff(null);setSelStaffLabel('');setSelDate(null);setSelTime(null);
    setAvailDates([]);setTimeSlots([]);setBookStaffId(null);setBookStaffName('');
    if(step<2) setStep(2); scrollTo(2);
  };
    ...
    <div style={{background:BG,minHeight:'100vh',fontFamily:ff,color:TX,maxWidth:480,margin:'0 auto',position:'relative',fontWeight:300}}>
      ...
      <div style={{padding:'0 16px 60px'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:0,marginBottom:28,padding:'0 6px'}}>
          {[1,2,3,4,5,6].map((n,i)=>{
            ...
            return(<React.Fragment key={n}>
              <div style={{width:20,height:20,borderRadius:'50%',background:active?P:'transparent',border:`1.5px solid ${active?P:CB}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.44rem',fontFamily:fp,fontWeight:500,color:active?'#fff':TLL,transition:'all 0.3s'}}>
          ...
          {services.length===0?(
            ...
            return(
              <div key={s.id} onClick={()=>pick(s.id)}
                ...
                {s.category&&<div style={{position:'absolute',top:0,right:0,background:P,color:'#fff',fontSize:'0.42rem',padding:'3px 10px',letterSpacing:'0.08em',fontWeight:400,borderRadius:'0 2px 0 3px'}}>{s.category}</div>}
                ...
                {s.variants&&s.variants.length>0&&sel&&(
                  <div style={{display:'flex',flexWrap:'wrap',gap:8,marginTop:14}}>
                    {s.variants.map((v,idx)=>{
                      ...
                      return(<button key={v.id||idx} onClick={e=>{e.stopPropagation();setVi(p=>({...p,[s.id]:idx}));}}
        ...
        <div ref={r2} style={crd(step>=2)}>
          ...
          <div style={{fontSize:'0.6rem',color:TL,marginBottom:16,fontWeight:300,lineHeight:1.7}}>可根據需要加選以下附加服務，非必選項目。</div>
          {addons.length===0?(
          ...
          <div style={{textAlign:'right',marginTop:8}}>
            <span style={{fontSize:'0.56rem',color:TL,fontWeight:300}}>已選 {ao.length} 項 {ao.length>0&&<span style={{fontFamily:fp}}>+${ap}</span>}</span>
          </div>
          {step===2&&(
        ...
        <div ref={r3} style={crd(step>=3)}>
          <SH n="3" z="選擇技師" e="SELECT TECHNICIAN"/>
          {/* 不指定 */}
          <div onClick={()=>pickStaff('any','不指定技師')}
            style={{display:'flex',alignItems:'center',gap:12,padding:'16px 14px',border:`1px solid ${selStaff==='any'?P:CB}`,borderRadius:3,marginBottom:10,cursor:'pointer',background:selStaff==='any'?PBG:'#fff',transition:'all 0.2s'}}>
            <div style={{width:36,height:36,borderRadius:'50%',background:selStaff==='any'?P:DV_BG,display:'flex',alignItems:'center',justifyContent:'center',border:`1.5px solid ${selStaff==='any'?PD:CB}`}}>
              <User size={16} color={selStaff==='any'?'#fff':TL} strokeWidth={1.5}/>
            </div>
            <div style={{flex:1}}>
              <div style={{fontSize:'0.76rem',fontWeight:selStaff==='any'?500:300}}>不指定技師</div>
              <div style={{fontSize:'0.54rem',color:TL,marginTop:2}}>系統自動安排可用技師</div>
            </div>
            {selStaff==='any'&&<Check size={16} color={P} strokeWidth={2}/>}
          </div>
          {staffList.map(s=>{
            const sel=selStaff===s.id;
            return(
              <div key={s.id} onClick={()=>pickStaff(s.id,s.name)}
                style={{display:'flex',alignItems:'center',gap:12,padding:'16px 14px',border:`1px solid ${sel?P:CB}`,borderRadius:3,marginBottom:10,cursor:'pointer',background:sel?PBG:'#fff',transition:'all 0.2s'}}>
                <div style={{width:36,height:36,borderRadius:'50%',background:sel?P:DV_BG,display:'flex',alignItems:'center',justifyContent:'center',border:`1.5px solid ${sel?PD:CB}`,fontFamily:fp,fontSize:'0.72rem',fontWeight:500,color:sel?'#fff':TM}}>
                  {s.name?.charAt(0)||'?'}
                </div>
                <div style={{flex:1}}>
                  <div style={{fontSize:'0.76rem',fontWeight:sel?500:300}}>{s.name}</div>
                </div>
                {sel&&<Check size={16} color={P} strokeWidth={2}/>}
              </div>
            );
          })}
          {staffList.length===0&&<div style={{textAlign:'center',padding:'10px 0',color:TL,fontSize:'0.68rem'}}>暫無可選技師</div>}
        </div>

        {/* ═══ Step 4: Date ═══ */}
        <div ref={r4} style={crd(step>=4)}>
          <SH n="4" z="選擇日期" e="SELECT DATE"/>
          {selStaff&&<div style={{fontSize:'0.58rem',color:P,marginBottom:16,fontWeight:400,background:PBG,padding:'10px 14px',borderRadius:3,border:`1px solid ${PL}`}}>
            已選技師：{selStaffLabel}
          </div>}
          {datesLoading?(
            <div style={{textAlign:'center',padding:'30px 0',color:TL,fontSize:'0.76rem'}}>載入可預約日期...</div>
          ):availDates.length===0?(
        ...
        <div ref={r5} style={crd(step>=5)}>
          ...
          {slotsLoading?(
            <div style={{textAlign:'center',padding:'30px 0',color:TL,fontSize:'0.76rem'}}>載入可用時段...</div>
          ):timeSlots.length===0?(
            <div style={{textAlign:'center',padding:'20px 0',color:TL,fontSize:'0.72rem'}}>
                  ...
                  {ss.slots.map(t=>{
                    ...
                    return(
                      <motion.div key={`${ss.staffId}-${t}`} whileTap={{scale:0.96}}
                        ...
                        style={{textAlign:'center',padding:'13px 6px',borderRadius:3,fontFamily:fc,fontSize:'0.88rem',letterSpacing:'0.06em',fontWeight:selected?500:400,border:`1px solid ${selected?PD:CB}`,cursor:'pointer',background:selected?P:'#fff',color:selected?'#fff':TX,transition:'all 0.2s'}}>
                        {t}
                      </motion.div>
                    );
                  })}
        ...
        <div ref={r6} style={crd(step>=6)}>
          <SH n="6" z="聯絡資料" e="CONTACT INFORMATION"/>
          <div style={{marginBottom:18}}>
            <div style={{fontSize:'0.6rem',color:TL,marginBottom:8,letterSpacing:'0.04em',fontWeight:300}}>您的姓名 <span style={{color:DR_TX}}>*</span></div>
          ...
          <div style={{marginBottom:18}}>
            <div style={{fontSize:'0.6rem',color:TL,marginBottom:8,letterSpacing:'0.04em',fontWeight:300}}>WhatsApp 電話 <span style={{color:DR_TX}}>*</span></div>
            <input value={ph} onChange={e=>setPh(e.target.value)} placeholder="e.g. 6000 0000" type="tel"
              style={{width:'100%',padding:'13px 16px',borderRadius:3,border:`1px solid ${CB}`,background:IB,fontSize:'0.76rem',fontFamily:ff,fontWeight:300,outline:'none',boxSizing:'border-box',color:TX}}/>
          </div>
          <div style={{marginBottom:4}}>
            <div style={{fontSize:'0.6rem',color:TL,marginBottom:8,letterSpacing:'0.04em',fontWeight:300}}>備註（選填）</div>
          ...
          <div style={{background:IB,borderRadius:3,padding:'18px 16px',border:`1px solid ${DV}`,marginBottom:18}}>
            ...
            <div style={{borderTop:`1px solid ${DV}`,paddingTop:12}}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',fontSize:'0.68rem',marginBottom:6}}>
                <span style={{color:TL,fontWeight:300}}>姓名</span><span style={{fontWeight:400}}>{nm||'—'}</span>
              </div>
              <div style={{display:'flex',justifyContent:'space-between',fontSize:'0.68rem',marginBottom:6}}>
                <span style={{color:TL,fontWeight:300}}>電話</span><span style={{fontWeight:400}}>{ph||'—'}</span>
              </div>
        ...
        {!canGo&&step>=6&&<div style={{textAlign:'center',fontSize:'0.58rem',color:DA_TX,marginBottom:14,fontWeight:300}}>請確保已填寫所有必填項目（*）</div>}
        ...
        <motion.button whileTap={canGo&&!submitLoading?{scale:0.97}:{}} onClick={handleSubmit} disabled={!canGo||submitLoading}
          ...
          <span style={{fontFamily:fc,fontSize:'0.54rem',fontWeight:300,letterSpacing:'0.2em',fontStyle:'italic',color:canGo?'rgba(255,255,255,0.7)':TLL}}>CONFIRM & SEND BOOKING REQUEST</span>
        </motion.button>

        <div style={{textAlign:'center',fontSize:'0.48rem',color:TLL,lineHeight:1.8,marginTop:8,fontWeight:300}}>提交後我們將透過 WhatsApp 與您確認預約</div>
      </div>
        ...
        {done&&(
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{scale:0.92,y:16}} animate={{scale:1,y:0}}
              ...
              <div style={{fontSize:'0.56rem',color:TL,marginBottom:20,lineHeight:1.7,fontWeight:300}}>我們將透過 WhatsApp 確認您的預約</div>
              <div style={{display:'flex',gap:10}}>
                <button onClick={()=>{setDone(false);setSid(null);setAo([]);setSelStaff(null);setSelStaffLabel('');setSelDate(null);setSelTime(null);setBookStaffId(null);setBookStaffName('');setNm('');setPh('');setRk('');setStep(1);setSubmitError('');window.scrollTo({top:0});}}
                  style={{flex:1,padding:'12px',borderRadius:3,border:`1px solid ${CB}`,background:'#fff',fontSize:'0.68rem',fontFamily:ff,cursor:'pointer',color:TM,fontWeight:300}}>再預約一次</button>
                <button onClick={()=>{
                  let msg=`Hi J.Lab 🌙\n\n我想確認預約：\n服務：${sv?.name}`;
...
function SH({n,z,e}){
  return(
    <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:20}}>
      ...
      <span style={{fontWeight:500,fontSize:'0.92rem'}}><span style={{fontFamily:fp}}>{n}.</span> {z}</span>
